...
export function computeSeriesState(games, teamIdA, teamIdB, winsNeeded) {
  ...
  for (const game of games) {
    ...
    if (game.game_status !== 'CLOSED') continue;

    const homeScore = parseInt(game.home_goals, 10) || 0;
    const awayScore = parseInt(game.away_goals, 10) || 0;

    if (homeScore === awayScore) continue; // Shouldn't happen in hockey playoffs but safety check
    ...
    // Stop counting after series is clinched
...
export function getWinsNeeded(format) {
  ...
  if (normalized.includes('singlegame') || normalized.includes('single')) return 1;
