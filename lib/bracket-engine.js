 ...
 */
export const BRACKET_STATE_VERSION = 4;
...
function initNationals(config) {
  return {
    ...
    // pods, seeds, schedule, days etc. will be added by admin once all divisions are decided
  };
}
      ...
      if (divState.roundRobin) {
        for (const podName of Object.keys(divState.roundRobin)) {
          ...
          for (const rrGame of pod.games) {
            ...
            if (apiGame) {
              rrGame.home_score = apiGame.home_score;
              rrGame.away_score = apiGame.away_score;
              rrGame.game_status = apiGame.game_status || 'final';
            }
          }
