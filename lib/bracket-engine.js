 ...
 */
export const BRACKET_STATE_VERSION = 35;
...
function initDineenCup(config) {
  return {
    ...
    formatInfo: config.formatInfo || '',
    mountainNote: config.mountainNote || '',
    teams: (config.teams || []).map((entry) => {
      // Support both string entries (legacy) and object entries (new with seed/qualification)
      if (typeof entry === 'string') {
        return { slot: entry, teamId: null, teamName: entry, name: entry };
      }
      return {
        slot: entry.slot || entry.name,
        teamId: entry.teamId || null,
        teamName: entry.name || entry.teamName,
        name: entry.name || entry.teamName,
    ...
    schedule: (config.schedule || []).map((g) => ({
      ...
      home_id: g.home_id || null,
      away_id: g.away_id || null,
      home_score: g.home_score != null ? g.home_score : null,
      away_score: g.away_score != null ? g.away_score : null,
