        ...
        rounds: format.rounds.map((round, roundIdx) => {
          ...
          return {
            ...
            matchups: round.matchups.map((matchup, matchupIdx) => {
              ...
              const teamA = typeof seedA === 'number' ? resolveTeam(seedA, divSeeds, divMappings) : null;
              const teamB = typeof seedB === 'number' ? resolveTeam(seedB, divSeeds, divMappings) : null;

              return {
                matchupId: `r${roundIdx}-m${matchupIdx}`,
                homeSeed: typeof seedA === 'number' ? seedA : null,
                awaySeed: typeof seedB === 'number' ? seedB : null,
