...
function updateNEConferenceWithGames(neConf, leagueGames) {
  ...
  for (const game of neConf.roundRobin.games) {
    if (game.game_status === 'final') continue; // already has scores
    ...
    if (status === 'CLOSED' || status === 'FINAL') {
      // Check if enriched scores are available (from game-scores KV cache)
      ...
      // If no enriched scores yet, leave as scheduled — update-scores cron will fill them
    }
  }
}
