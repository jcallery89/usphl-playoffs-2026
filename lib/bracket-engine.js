 ...
 */
export const BRACKET_STATE_VERSION = 23;
...
function updateNEConferenceWithGames(neConf, leagueGames) {
  if (!neConf?.roundRobin?.games) return;
  const gameMap = new Map();
  for (const g of leagueGames) {
    gameMap.set(String(g.game_id), g);
  }
  for (const game of neConf.roundRobin.games) {
    ...
    if (status === 'CLOSED' || status === 'FINAL') {
      // Schedule API doesn't include scores for CLOSED games,
      // but we can mark them as final if scores were already populated
      if (game.home_score != null && game.away_score != null) {
        game.game_status = 'final';
        game.winner_id = game.home_score > game.away_score ? game.home_id : game.away_id;
      } else {
        game.game_status = 'final_pending';
      }
    }
  }
}
