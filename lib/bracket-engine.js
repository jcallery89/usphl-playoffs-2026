...
export function initializeBracketState(bracketConfig, initialSeeds, teamMappings) {
  ...
  for (const league of ['Premier', 'Elite', 'NCDC']) {
    ...
    for (const [division, config] of Object.entries(leagueConfig)) {
      ...
      // Collect all BYE seeds across rounds for pre-populating later rounds
      const allByeSeeds = new Set();
      for (const r of format.rounds) {
        for (const byeSeed of (r.byes || [])) {
          allByeSeeds.add(byeSeed);
        }
      }
        ...
        rounds: format.rounds.map((round, roundIdx) => {
          ...
          return {
            ...
            matchups: round.matchups.map((matchup, matchupIdx) => {
              ...
              // Resolve teams for Round 1, and for BYE teams in later rounds
              // (BYE teams are known to advance, so we can pre-populate them).
              ...
              const canResolveA = (typeof seedA === 'number' && (isFirstRound || allByeSeeds.has(seedA))) || isPodSeedA;
              const canResolveB = (typeof seedB === 'number' && (isFirstRound || allByeSeeds.has(seedB))) || isPodSeedB;
